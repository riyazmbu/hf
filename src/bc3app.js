...
const intradayLosers = rows
  ...
  .slice(0, 10)
     const losers = rows
  .map((r) => {
    const name = String(r[5] || '').trim()      // G
    const price = r[6] !== undefined && r[6] !== '' ? r[6] : '--' // H
    const rawPct = String(r[7] || '').trim()    // I
  ...
  .slice(0, 4)
     const gainers = rows
  .map((r) => {
    const name = String(r[0] || '').trim()      // B
    const price = r[1] !== undefined && r[1] !== '' ? r[1] : '--' // C
    const rawPct = String(r[2] || '').trim()    // D
  ...
  return (
    <>
        ...
        <div className="hero-right">
          <div className="hero-right-hud">
            <div className="hud-sentiment-title">
              <span>Active Matrix Sentiment</span>
              <span className="sentiment-percentage">68% Bullish</span>
            </div>
            <div className="sentiment-bar-wrapper">
              <div className="sentiment-bullish-fill"></div>
            </div>
            <div className="sentiment-metrics">
              <span style={{ color: 'var(--primary-green)' }}>1,876 Advances</span>
              <span style={{ color: 'var(--bearish-red)' }}>756 Declines</span>
            </div>

            <div className="mini-chart-mockup">
              <div className="mini-chart-row">
                <span className="mini-chart-name">NIFTY Velocity</span>
                <span className="mini-chart-spark">▲ High Velocity</span>
              </div>
              <div className="mini-chart-row">
                <span className="mini-chart-name">Options PCR Vector</span>
                <span className="mini-chart-spark" style={{ color: 'var(--primary-blue)' }}>0.88 Stable</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Real-time Movers Panels */}
      {/* <div className="section-title-wrapper">
        <div className="section-title">Market Structural Scans</div>
        <a href="#" className="view-all-link">View All Stocks →</a>
      </div>

      <section className="movers-matrix-grid">
        <div className="mover-deck-panel">
          <div className="mover-deck-title" style={{ color: 'var(--bullish-green)' }}>🟢 Long Build Up</div>
          <div id="longbuild"><AssetPanel items={movers.longbuild} showPrice={false} /></div>
        </div>
        <div className="mover-deck-panel">
          <div className="mover-deck-title" style={{ color: 'var(--bearish-red)' }}>🔴 Short Build Up</div>
          <div id="shortbuild"><AssetPanel items={movers.shortbuild} showPrice={false} /></div>
        </div>
        <div className="mover-deck-panel">
          <div className="mover-deck-title" style={{ color: 'var(--primary-blue)' }}>🔵 Short Covering</div>
          <div id="shortcovering"><AssetPanel items={movers.shortcovering} showPrice={false} /></div>
        </div>
      </section> */}
      ...
      <section className="sub-movers-grid">
        <div className="mover-deck-panel">
         <div className="mover-deck-title" style={{ color: 'var(--bullish-green)' }}>Top Gainers</div>
          <div id="gainers"><AssetPanel items={movers.gainers} showPrice /></div>
        </div>
        <div className="mover-deck-panel">
          <div className="mover-deck-title" style={{ color: 'var(--bearish-red)' }}>Top Losers</div>
          <div id="losers"><AssetPanel items={movers.losers} showPrice /></div>
        </div>
        <div className="mover-deck-panel">
        ...
        <div className="mover-deck-panel">
          ...
          <div id="intraday-loosers"><AssetPanel items={movers.intradayLosers} showPrice /></div>
        </div>
        {/* <div className="mover-deck-panel">
          <div className="mover-deck-title">52 Week High</div>
          <div id="yearly-high"><AssetPanel items={movers.yearlyHigh} showPrice /></div>
        </div> */}
      </section>
      ...
      {/* <div className="section-title-wrapper">
        <div className="section-title">Our Free YouTube Resources</div>
        <a href="#" className="view-all-link">View All Videos →</a>
      </div>
        ...
        {VIDEOS.map((v, i) => (
          <div className="video-card" key={i}>
            <div
              ...
              style={{
                background: `linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.6)), url('${v.img}') center/cover`,
              }}
            >
              <div>{v.title}</div>
              <span className="video-duration">{v.duration}</span>
            </div>
            <div className="video-meta-desc">{v.desc}</div>
          </div>
        ))}
      </section> */}
...
function handleAdminLogout() {
  ...
  {/* Catch-all route for /blog and /blog/:slug */}
  <Route 
    path="/blog/:slug?" 
    element={
      <BlogModule 
  ...
  />
        ...
        {/* <section className="newsletter-banner">
          <div className="banner-left">
            <h3>Never Miss a Key Intraday Breakout</h3>
            <p>Subscribe to our high-priority trade setups and strategic engine alerts delivered instantly to your inbox.</p>
          </div>
          <div className="banner-right-form">
            <input type="email" className="banner-input" placeholder="Enter your email address" />
            <button className="btn-green" style={{ padding: '12px 24px', boxShadow: 'none', whiteSpace: 'nowrap' }}>
              Join Stream
            </button>
          </div>
        </section> */}
